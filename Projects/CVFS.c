...
#include<stdlib.h>
#include<string.h>
//#include<unistd.h>  //Linux command
...
#define MAXFILESIZE 10 //2048byte

#define REGULAR 1		//type of files we use 1
#define SPECIAL 2
...
typedef struct superblock
{
	int TotalInodes;	
	int FreeInode;
}SUPERBLOCK,*PSUPERBLOCK;   //superblock is used to mantain size of Inodes

typedef struct inode
{
	char FileName[50];       //file name storer
	int InodeNumber;		//inode number
	int FileSize;			//1024 byte
	int FileActualSize;		//regular
	int FileType;			//type of file
	char *Buffer;			//adress 1024 byte memory's,it is a char*  //total size of structure is 86 byte
	int LinkCount;			//linking cha count
...
{
	int readoffset;			//kuthun vachaych
	int writeoffset;		//kuthun lihaych
	int count;				//1 always
	int mode;				//mode of file
	PINODE ptrinode;		//pointer
}FILETABLE,*PFILETABLE;

typedef struct ufdt
{
	PFILETABLE ptrfiletable;	//ufdt structure banavale
}UFDT;
...
void CloseFileByName(int fd) //close 0
{
...
int main() 
{
	...
	while(1)
	{
		...
		else if(count == 2)
		{
			...
			else if(strcmp(command[0],"rm") == 0)//remove file //unlink vaparlay system call
			{
			...
			else if(strcmp(command[0],"write") == 0)
			{
				...
				scanf("%[^\n]",arr); //ABCDEFGHIJKLMNOPQRSTUVXWXYZ
				...
				if(ret>0)
				{
					printf("Sucess:%d bytes successfully written",ret);
				}
				if(ret==-4)
				{
					printf("ERROR:There bis no sufficient memory Available");
				}
			}
		...
		else if(count == 3)
		{
			...
			else if(strcmp(command[0],"read") == 0)
			{
				...
				if(ret > 0)
				{
					write(1,ptr,ret);  //1 is std out,0 std in //write is like a print f
				}
				continue;
			}
			else	
